...
export default function ReportsPage() {
  ...
  return <main className="app-shell"><Header /><div className="content-wrap subpage"><div className="page-intro"><p className="eyebrow">日报归档</p><h1>所有新闻日报</h1><p>按日期查看已经生成的日报和当天聚合的重点事件。</p></div>{loading ? <div className="loading-inline">正在加载日报列表…</div> : <div className="report-calendar">{reports.map((report) => <Link className="report-calendar-item" href={`/report/${report.date}`} key={report.date}><span className="report-date">{report.date}</span><strong>{report.report.events.length}</strong><small>个事件</small><p>{report.overview || '查看当天 AI 新闻摘要'}</p></Link>)}</div>}{!loading && !reports.length && <section className="empty-panel"><h2>暂无日报</h2><p>日报生成后会显示在这里。</p></section>}</div><Assistant /></main>;
}

...
export default function TrendsPage() {
  ...
  const visibleTech = trend.hot_tech.slice(0, 8);

  const visibleEvents = showAllEvents ? trend.events : trend.events.slice(0, 5);
  return <main className="app-shell"><Header /><div className="content-wrap subpage"><div className="page-intro compact-intro"><h1>趋势与方向</h1><p>看看最近一个月，AI 讨论的重点正在向哪里移动。</p></div><section className="metrics-grid">{[['新闻总数', trend.total_news], ['重要新闻', trend.important_news], ['主要事件', trend.main_events], ['热门方向', trend.hot_directions]].map(([label, value]) => <div className="metric" key={label as string}><span>{label}</span><strong>{value}</strong><small>过去 30 天</small></div>)}</section><section className="section-grid"><div className="panel tech-panel"><div className="section-heading"><div><h2>热门技术</h2><p className="panel-note">点击技术方向查看相关新闻</p></div></div><div className="tech-list">{visibleTech.map((tech, index) => <Link className="tech-row tech-link" href={`/important?topic=${encodeURIComponent(tech.name)}`} key={tech.name}><span>{tech.name}</span><div className="bar-track"><div className={`bar bar-${index}`} style={{ width: `${Math.max(12, tech.count / max * 100)}%` }} /></div><b>{tech.count} 篇</b><small>{Math.round(tech.count / max * 100)}%</small></Link>)}</div></div><div className="panel"><div className="section-heading"><div><h2>重要事件</h2></div></div>{visibleEvents.map((event) => { const report = eventReport(event.title); const detail = report?.report.events.find((item) => item.title === event.title); const isExpanded = expandedEvent === event.title; return <div className="event-entry" key={`${event.rank}-${event.title}`}><button className="event-row event-link" onClick={() => setExpandedEvent(isExpanded ? null : event.title)}><span>0{event.rank}</span><p>{event.title}<small>{isExpanded ? '收起详情 ↑' : '查看事件详情 ↓'}</small></p></button>{isExpanded && <div className="event-detail"><p>{detail?.summary || '暂无事件摘要。'}</p>{detail?.why_it_matters && <p><strong>关注理由：</strong>{detail.why_it_matters}</p>}</div>}</div>; })}{trend.events.length > 5 && <button className="show-all-button" onClick={() => setShowAllEvents((current) => !current)}>{showAllEvents ? '收起其他事件' : `展开其余 ${trend.events.length - 5} 个事件`}</button>}</div></section></div></main>;
}
